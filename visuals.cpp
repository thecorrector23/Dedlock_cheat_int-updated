    ...
    for (auto& cache : entityCache) {
        ...
        if (esp_health_enabled) {
            if (health_display_mode == HEALTH_BAR) {
                ...
                draw_list->AddText(ImVec2(screen_pos_foot.X - 40.f, screen_pos_foot.Y), ImColor(255, 255, 255), health_info.c_str());
            }
        }

        if (esp_show_name && !cache.entityName.empty()) {
            draw_list->AddText(ImVec2(screen_pos_head.X, screen_pos_head.Y - 15.0f), ImColor(255, 255, 255), cache.entityName.c_str());
        }

        if (esp_show_address) {
            draw_list->AddText(ImVec2(screen_pos_foot.X, screen_pos_foot.Y + 20.0f), ImColor(255, 255, 255), cache.entityAddressStr.c_str());
        }
    }
}
...
void Visuals1::RenderSettingsMenu() {
    ...
    if (health_settings_visible) {
        ...
        ImGui::RadioButton("Health Bar", health_display_mode == HEALTH_BAR);
        ImGui::RadioButton("Health Number", health_display_mode == HEALTH_NUMBER);
        ImGui::Unindent();
    }
}
